 ...
 */
package com.fvogel.broadcomcc1.advice;
...
/**
 * Handles turning exceptions into RFC-7807 problem/json responses, so instead
 * of an exception and its stack trace leaking back to the client, an RFC-7807
 * problem description is returned instead.
 */
...
public class GlobalExceptionHandler implements ProblemHandling, ErrorWebExceptionHandler {
	...
	private Mono<Problem> problemDescription(String title, Throwable throwable, Status status) {
		Problem problem = Problem.builder().withStatus(status).withDetail(throwable.getMessage()).withTitle(title)
				.build();
