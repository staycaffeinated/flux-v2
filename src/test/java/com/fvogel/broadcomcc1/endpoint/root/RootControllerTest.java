...
class RootControllerTest {
	...
	void testMonoResponse() {
		Flux<String> msg = webClient.get().uri("/mono").exchange().expectStatus().isOk().returnResult(String.class)
				.getResponseBody().log();
	...
	/**
	 * An example of handling errors in Flux streams. This example illustrates the
	 * technique and does not exercise the root controller.
	 */
	@Test
	void testFluxErrorHandling() {
		Flux<String> stringFlux = Flux.just("A", "B", "C")
				.concatWith(Flux.error(new UnprocessableEntityException("Exception Occurred")))
				.concatWith(Flux.just("D")).onErrorResume((e) -> { // this block gets executed
	...
	/**
	 * An example of a flux stream returning a default value when an error occurs in
	 * the stream. This example illustrates the technique and does not exercise the
	 * root controller.
	 */
	@Test
	void testFluxErrorHandling_OnErrorReturn() {
		...
		Flux<String> stringFlux = Flux.just("A", "B", "C")
				.concatWith(Flux.error(new RuntimeException("Exception Occurred"))).concatWith(Flux.just("D"))
	 ...
	 * This example illustrates the technique and does not exercise the root
	 * controller.
	 */
	@Test
	void testFluxErrorHandling_OnErrorMap() {
		...
		StepVerifier.create(stringFlux.log()).expectSubscription().expectNext("A", "B", "C")
				.expectError(UnprocessableEntityException.class).verify();

	}

	/**
	 * An example of a flux stream using retry when an exception occurs. This
	 * example illustrates the technique and does not exercise the root controller.
	 */
	@Test
	void whenErrorOccurs_shouldContinueUninterrupted() {
		...
		Flux<String> stringFlux = Flux.just("A", "B", "C")
				.concatWith(Flux.error(new RuntimeException("Exception Occurred"))).concatWith(Flux.just("D"))
				.onErrorMap((e) -> new UnprocessableEntityException(e)).retry(2);

		StepVerifier.create(stringFlux.log()).expectSubscription().expectNext("A", "B", "C").expectNext("A", "B", "C")
				.expectNext("A", "B", "C").expectError(UnprocessableEntityException.class).verify();
	}
}